...
featfile = u_utt+'/dvector.ark'
vadfile = u_utt+'/fbank_vad.ark'
threshold = 0.9194
...
for index, utt in enumerate(content):
	...
	times, scores = fix_slid(utt_vad, utt_vad_index, float(win_len), float(win_shift))
...
ftimes = file(u_utt+'/dvec_time.tag','w')
for i in times_c[0]:
        ftimes.write(str(i)+' ')

fscores = file(u_utt+'/dvec_score.tag','w')
for i in scores_c[0]:
        fscores.write(str(i)+' ')

#fref = file(u_utt+'/dvec_ref.tag','w')
#for i in ref_c[0]:
...
#roc(scores, ref, ft)
'''
for i in [0.01, 0.05, 0.1, 0.2, 0.25]:
	times, scores = fix_slid(utt_vad, utt_vad_index, i, 0.01)
	roc(scores, ref, ft)

plt.legend(('0.01 s', '0.05 s', '0.1 s', '0.2 s', '0.25 s'), shadow=True, loc=(0.7, 0.7))
ltext = plt.gca().get_legend().get_texts()
plt.plot([0, 1], [0, 1], '--', color='red')
plt.show()
'''
